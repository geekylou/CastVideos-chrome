...
for (dirpath, dirnames, filenames) in os.walk(path):
    if directory_arg:
        ...
        if directory_arg.rfind('/') > 0:
            directories.append( {"title" : ".." , "icon" : "/icons/go-previous-3.png", "directory" : [directory_arg[:directory_arg.rfind('/')]] } )
        else:
            directories.append( {"title" : ".." , "icon" : "/icons/go-previous-3.png", "directory" : [""] } )        
    else:
        base_arg = ""
    ...
    for dirname in dirnames:
            directories.append( {"title" : dirname , "icon" : "/icons/document-open-folder.png", "directory" : [base_arg + dirname] } )
            
    filenames.sort()
    
    for filename in filenames:
        if filename.endswith(".mp4") or filename.endswith(".mov") or filename.endswith(".mkv"):
            videos.append( {"title" : filename , "icon" : "/icons/video-x-generic.png", "sources" : [BASE_URL + base_arg + "/" + filename] } )
